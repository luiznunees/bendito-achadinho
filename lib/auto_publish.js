...
const { insertLog } = require("./publish_log");
...
async function runAutoPublish() {
  ...
  const spreadMode = cfg("AUTOPUBLISH_SPREAD_MODE", "burst").toLowerCase();
  const spreadMinutes = numCfg("AUTOPUBLISH_SPREAD_MINUTES", 10);
  ...
  // Intervalo entre as ofertas:
  //  - burst  : todas juntas, com AUTOPUBLISH_SEND_DELAY (segundos) entre elas
  //  - spread : espaçadas, uma a cada AUTOPUBLISH_SPREAD_MINUTES (minutos)
  const delayMs = spreadMode === "spread"
    ? Math.max(1, spreadMinutes) * 60 * 1000
    : Math.max(0, delayBetween) * 1000;
  ...
  for (let i = 0; i < offers.length; i++) {
    ...
    try {
      ...
      // Só espera ENTRE uma oferta e a próxima (não após a última).
  ...
  fireAndForgetLog({
    ...
    shopeeItemIds: offers.filter((o) => !summary.errors.includes(String(o.itemId))).map((o) => Number(o.itemId) || null).filter((x) => x !== null),
  });
  return summary;
}
