...
async function ensureDayPlan(runDate) {
  try {
    ...
    const offers = await collectOffers(size);
    if (offers.length === 0) return [];

    await replaceDayPlan(runDate, offers);
    console.log(`auto_publish: plano do dia ${runDate} montado com ${offers.length} de ${size} ofertas`);
    return offers;
  } catch (err) {
