...
const { insertProduct, findActiveByImage, getRecentTitles } = require("./db");
...
function getKeywords() {
  return cfg("AUTOPUBLISH_KEYWORDS", "terço,terço de madeira,rosário,medalha,escapulário,imagem de santo,imagem de nossa senhora,imagem de são josé,estátua sagrada família,quadro religioso católico,porta-retrato religioso,camiseta católica,caneca católica,chaveiro católico,bíblia,bíblia de estudo,catecismo,camiseta gospel,caneca gospel,livro evangélico,chaveiro gospel,caneca cristã,camiseta cristã,pulseira cristã,livro de oração")
    .split(",")
    .map((k) => k.trim())
    .filter(Boolean);
}
...
const DEFAULT_BLOCK_TERMS =
  ...
  // fora do nicho mas que colam por causa de "medalha"/"cruz" no título
  // (ex.: sandália com fivela/medalhão dourado)
  "sandalia,rasteira,rasteirinha,chinelo,sapato,sapatilha,tenis,salto alto," +
  // itens genéricos ruins apontados pelo curador (canetinha de marca-texto,
  // caneca personalizada clichê "sou filho(a) do rei")
  "marca texto,marca-texto,filho do rei,filha do rei";
...
// Testa se `term` aparece em `text` como PALAVRA (nao substring solta).
// Sem isso, termo curto tipo "fe" (de "fe") batia em "feminina",
// "confeccionado" etc. -- aprovava produto fora do nicho so por acaso de
// substring. `term` ja deve vir normalizado (sem acento).
...
function passesNicheFilter(p) {
  ...
  return terms.some((t) => hasTerm(title, t));
}
...
function nicheStrength(p) {
  ...
  const hits = terms.filter((t) => hasTerm(title, t)).length;
...
async function searchKeyword(keyword) {
  const sortType = parseInt(cfg("AUTOPUBLISH_SORT_TYPE", "2"), 10); // 2 = melhor sobrevivência aos filtros (nível bom de desconto/vendas)
...
async function collectOffers(limit) {
  ...
  // O filtro acima só pega o MESMO itemId. Vendedores diferentes (ou o
  // mesmo vendedor em outro anúncio) publicam o "mesmo" produto com
  // itemId novo — daí a mesma caneca/kit reaparecer poucas horas depois.
  // Aqui comparamos por SIMILARIDADE DE TÍTULO contra o que foi
  // cadastrado no site nos últimos dias (titleSimilar, já usado no plano).
  let recentTitles = [];
  if (noRepeatDays > 0) {
    try {
      recentTitles = await getRecentTitles(noRepeatDays);
    } catch (err) {
      console.warn("auto_publish: falha ao buscar títulos recentes:", err.message);
    }
  }
  ...
  for (const keyword of keywords) {
    ...
    for (const p of nodes) {
      ...
      const candidateTitle = p.productName || p.title || "";
      if (candidateTitle && recentTitles.some((t) => titleSimilar(candidateTitle, t))) continue;
...
async function runAutoPublish(opts = {}) {
  ...
  const summary = { published: 0, saved: 0, skipped: 0, errors: [], errorDetails: [] };
    ...
    try {
      ...
      try {
        if (sendImage && offer.imageUrl) {
          // Marca d'água na foto (protege contra repost sem crédito). Se
          // falhar por qualquer motivo, cai pra foto original sem marca —
          // nunca perde o envio por causa disso.
          const watermarked = await watermarkImage(offer.imageUrl);
          await sendWhatsAppImage(groupId, watermarked || offer.imageUrl, message);
        } else {
        ...
        summary.errorDetails.push(String(err.message).slice(0, 160));
      }
    } catch (err) {
    ...
    detail:
      summary.published > 0
        ...
        : summary.errors.length > 0
          ? `${summary.errors.length} falha(s) no envio${summary.errorDetails[0] ? `: ${summary.errorDetails[0]}` : ""}`
          : "nada enviado",
...
module.exports = { runAutoPublish, findOffersToPublish, collectOffers, buildDiversePlan, buildGroupMessage, passesFilters, passesNicheFilter, runGreeting, getGreetingMessage, getGreetingImageUrl, loadPipelineSettings, cfg, rankOfferScore, pickCandidate, ensureDayPlan };
