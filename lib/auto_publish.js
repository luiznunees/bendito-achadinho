...
function priceScore(p) {
  ...
  if (price <= 0) return 0;
  if (price <= 5) return 0.2;
  if (price < 15) return 0.4 + ((price - 5) / 10) * 0.6;
...
function rankOfferScore(p) {
  return (
    numCfg("AUTOPUBLISH_RANK_W_NICHE", 0.35) * nicheStrength(p) +
    numCfg("AUTOPUBLISH_RANK_W_SALES", 0.25) * salesScore(p) +
    numCfg("AUTOPUBLISH_RANK_W_DISCOUNT", 0.15) * discountScore(p) +
    numCfg("AUTOPUBLISH_RANK_W_COMMISSION", 0.15) * commissionScore(p) +
    numCfg("AUTOPUBLISH_RANK_W_PRICE", 0.1) * priceScore(p)
  );
}
...
async function runAutoPublish(opts = {}) {
  ...
  if (dryRun) {
    console.log(`auto_publish (DRY RUN): ${offers.length} candidatas; sorteada (top ${Math.max(1, parseInt(cfg("AUTOPUBLISH_RANK_POOL", "8"), 10) || 8)}):`);
...
module.exports = { runAutoPublish, findOffersToPublish, buildGroupMessage, passesFilters, passesNicheFilter, runGreeting, getGreetingMessage, getGreetingImageUrl, loadPipelineSettings, cfg, rankOfferScore, pickCandidate };
