...
// ItemIds da Shopee realmente enviados nos últimos `days` dias
// (status "sent"). Usado para nunca repetir a mesma oferta no grupo.
// Falha aberto: em erro, devolve conjunto vazio (não bloqueia o envio).
async function getSentShopeeItemIds(days = 3) {
  try {
    const supabase = getClient();
    const since = new Date(Date.now() - days * 24 * 3600 * 1000).toISOString();
    const { data, error } = await supabase
      .from("auto_publish_log")
      .select("shopee_item_ids")
      .eq("status", "sent")
    ...
    for (const row of data || []) {
      if (!Array.isArray(row.shopee_item_ids)) continue;
      for (const id of row.shopee_item_ids) {
        if (id != null && String(id).trim() !== "") ids.add(String(id));
      }
    }
    return ids;
  } catch (err) {
    console.warn("publish_log.getSentShopeeItemIds:", err.message);
    return new Set();
  }
}
...
module.exports = { insertLog, getLogsOfDay, getRecentLogs, getSentShopeeItemIds, acquireSlot, finishSlot };