...
const NICHE_JUDGE_TEMPLATE = (title) => `Você é curador de ofertas para o "Bendito Achadinho", um grupo de achadinhos (achados baratos) para público CRISTÃO (católico e evangélico).
Muitos vendedores colocam palavras como "evangélica", "cristã", "gospel", "cruz", "jesus" no título de produtos GENÉRICOS só para aparecer em buscas.
Analise o produto abaixo e responda se ele é REALMENTE do nicho cristão/religioso — ou seja, um item devocional, de fé, ou com arte/estampa religiosa, e não um produto genérico com palavra religiosa de enfeite. Exemplo de item de enfeite: "blusa de academia moda evangélica tapa bumbum".

Responda apenas uma palavra: SIM ou NÃO.

Produto: "${title}"`;

// Juiz de nicho: verdadeiro se o produto for realmente cristão.
// Fail-open: se a IA falhar ou não responder claro, ACEITA (não bloqueia).
async function verifyNiche(rawTitle) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey || !rawTitle) return true;
  const text = await callGemini(NICHE_JUDGE_TEMPLATE(rawTitle), apiKey);
  if (!text) return true;
  const firstToken = String(text)
    .replace(/[^\p{L}\s]/gu, " ")
    .trim()
    .split(/\s+/)[0]
    .toLowerCase();
  return !/^n[aã]o$/.test(firstToken);
}
...
module.exports = { rewriteTitle, generateHeadline, generateOfferTexts, verifyNiche };
